...
simulates two-site federated RNA-seq data.
...
- Tile labels showing the p value (or adjusted p value, if `p_adj` is set), in scientific notation with two decimal places (e.g. `3.12e-04`), can be enabled with `label = TRUE`;
...
ds.plotDrivers(
  ...
  n_pc                 = 10,
  sig_cutoff           = 0.06,
  label                = TRUE,         # print p values on tiles 
  max_col              = 30,           # fix colour scale maximum
...
```
<p>&nbsp;</p>
![](dsDriversPCA_combined2.png){width=70% style="display: block; margin: auto;"}
...
# Columns: Feature, PC, pvalue, Significant, plus pvalue_adj if p_adj is set
head(res$results)
